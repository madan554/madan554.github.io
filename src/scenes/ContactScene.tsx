import { useMotion } from '../motion/MotionRoot'

function GitHubIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
    </svg>
  )
}

function LinkedInIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  )
}

function SubstackIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M22.539 8.242H1.46V5.406h21.08v2.836zM1.46 10.812V24L12 18.11 22.54 24V10.812H1.46zM22.54 0H1.46v2.836h21.08V0z" />
    </svg>
  )
}

function MediumIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true">
      <path d="M13.54 12a6.8 6.8 0 0 1-6.77 6.82A6.8 6.8 0 0 1 0 12a6.8 6.8 0 0 1 6.77-6.82A6.8 6.8 0 0 1 13.54 12Zm7.42 0c0 3.54-1.51 6.42-3.38 6.42s-3.39-2.88-3.39-6.42 1.52-6.42 3.39-6.42 3.38 2.88 3.38 6.42ZM24 12c0 3.17-.53 5.75-1.19 5.75S21.62 15.17 21.62 12s.53-5.75 1.19-5.75S24 8.83 24 12Z" />
    </svg>
  )
}

export function ContactScene() {
  const { setCursor } = useMotion()

  return (
    <section className="contact-scene" id="contact">
      <p className="kicker">Hyderabad or remote</p>
      <h2>Let’s talk.</h2>
      <p>
        Open to Python backend, GenAI application, and Python full-stack roles.
      </p>
      <ul className="links">
        <li>
          <a
            href="mailto:Madanmohan46660@gmail.com"
            onPointerEnter={() => setCursor('WRITE')}
            onPointerLeave={() => setCursor('')}
          >
            Madanmohan46660@gmail.com
          </a>
        </li>
        <li>
          <a href="tel:+916305581858">+91 63055 81858</a>
        </li>
      </ul>
      <p className="find-label">Find me on</p>
      <ul className="find-me">
        <li>
          <a
            href="https://github.com/madan554"
            target="_blank"
            rel="noreferrer"
            onPointerEnter={() => setCursor('GITHUB')}
            onPointerLeave={() => setCursor('')}
          >
            <GitHubIcon />
            GitHub
          </a>
        </li>
        <li>
          <a
            href="https://www.linkedin.com/in/madan-mohan-reddy-mucheli/"
            target="_blank"
            rel="noreferrer"
            onPointerEnter={() => setCursor('LINKEDIN')}
            onPointerLeave={() => setCursor('')}
          >
            <LinkedInIcon />
            LinkedIn
          </a>
        </li>
        <li>
          <a
            href="https://substack.com/@madanmohan715024/posts"
            target="_blank"
            rel="noreferrer"
            onPointerEnter={() => setCursor('SUBSTACK')}
            onPointerLeave={() => setCursor('')}
          >
            <SubstackIcon />
            Substack
          </a>
        </li>
        <li>
          <a
            href="https://medium.com/@mohan0908"
            target="_blank"
            rel="noreferrer"
            onPointerEnter={() => setCursor('MEDIUM')}
            onPointerLeave={() => setCursor('')}
          >
            <MediumIcon />
            Medium
          </a>
        </li>
      </ul>
      <footer>Built with React · Hosted on GitHub Pages · {new Date().getFullYear()}</footer>
    </section>
  )
}
